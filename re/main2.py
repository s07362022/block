...
def main_strat(account,score2,cnum,nonum,f,mode,mode_1_1,time_flag,flag,vc,timer1,start_time): 
    cnum = int(cnum)
    col_list = ['帳號','偏癱測','模式','個數','完成60秒','起始時間','結束時間'] ##############12/27
    ...
    if time_flag==True :
        ...
        if mode =='1':
            ...
            if int(alltime)>timer1:
                ...
                QQ = (account,score2,alltime,mode,mode_1_1,str(nonum),str(cnum),str(box_count),t,start_timeall,end_time_all,"Y")
                cursor.execute(SS,QQ)
                connection.commit()
                dic_df  = {"帳號":account,'偏癱測':mode_1_1,'模式':mode,'個數':str(box_count),'完成60秒':"Y",'起始時間':start_timeall,'結束時間':t}
                df.append(dic_df,ignore_index=True)
            elif (alltime %1 == 0):
                t = str( datetime.now().strftime(ISOTIMEFORMAT))
                posi='0'
                ...
                dic_df  = {"帳號":account,'偏癱測':mode_1_1,'模式':mode,'個數':str(box_count),'完成60秒':"N",'起始時間':start_timeall,'結束時間':end_time_all}
                df.append(dic_df,ignore_index=True)
            #score2 = 0
            #except:
                #pass
            ...
            if int(alltime)>timer1:#60
                ...
                t = str( datetime.now().strftime(ISOTIMEFORMAT)) ################## nounm 數值有錯
                SS = "INSERT INTO `blockbd2`(`account`,`Score`,`Time`,`Mode`,`rl`,`nonum`,`cnum`,`boxes`,`time_`,`strat_time`,`end_time`,`sixs`) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
                QQ = (account,score2,alltime,mode,mode_1_1,str(nonum),str(cnum),str(box_count),t,start_timeall,t,"Y")
                cursor.execute(SS,QQ)
                dic_df  = {"帳號":account,'偏癱測':mode_1_1,'模式':mode,'個數':str(box_count),'完成60秒':"Y",'起始時間':start_timeall,'結束時間':t}
                df.append(dic_df,ignore_index=True)
