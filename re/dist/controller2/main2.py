...
ISOTIMEFORMAT = '%H%M%S' #ISOTIMEFORMAT
class_names = []
...
def mode11(x,y,w,h,position_num):   # 左手偵測 IOU 面積範圍
    ...
    if ((x<lxy2[0]<x+w) and (y<lxy2[1]<y+h) ):
        box2=[lxy2[0],lxy2[1],lxy2[0]+20,lxy2[1]+20]
        iou_s = Iox(box1,box2)
        if iou_s >0.5:
            position_num.append('2')
        return position_num
    if ((x<lxy3[0]<x+w) and (y<lxy3[1]<y+h) ):
        box2=[lxy3[0],lxy3[1],lxy3[0]+20,lxy3[1]+20]
        iou_s = Iox(box1,box2)
        if iou_s >0.5:
            position_num.append('3')
        return position_num
    if ((x<lxy4[0]<x+w) and (y<lxy4[1]<y+h) ):
        box2=[lxy4[0],lxy4[1],lxy4[0]+20,lxy4[1]+20]
        iou_s = Iox(box1,box2)
        if iou_s >0.5:
            print('第 4 格 偵測到 ')
    ...
    if ((x<lxy5[0]<x+w) and (y<lxy5[1]<y+h) ):
        box2=[lxy5[0],lxy5[1],lxy5[0]+20,lxy5[1]+20]
        iou_s = Iox(box1,box2)
        if iou_s >0.5:
            print('第 5 格 偵測到 ')
    ...
    if ((x<lxy6[0]<x+w) and (y<lxy6[1]<y+h) ):
        box2=[lxy6[0],lxy6[1],lxy6[0]+20,lxy6[1]+20]
        iou_s = Iox(box1,box2)
        if iou_s >0.5:
            print('第 6 格 偵測到 ')
    ...
    if ((x<lxy7[0]<x+w) and (y<lxy7[1]<y+h) ):
        box2=[lxy7[0],lxy7[1],lxy7[0]+20,lxy7[1]+20]
        iou_s = Iox(box1,box2)
        if iou_s >0.5:
            print('第 7 格 偵測到 ')
    ...
    if ((x<lxy8[0]<x+w) and (y<lxy8[1]<y+h) ):
        box2=[lxy8[0],lxy8[1],lxy8[0]+20,lxy8[1]+20]
        iou_s = Iox(box1,box2)
        if iou_s >0.5:
            print('第 8 格 偵測到 ')
    ...
    if ((x<lxy9[0]<x+w) and (y<lxy9[1]<y+h) ):
        box2=[lxy9[0],lxy9[1],lxy9[0]+20,lxy9[1]+20]
        iou_s = Iox(box1,box2)
        if iou_s >0.5:
            print('第 9 格 偵測到 ')
    ...
    if ((x<lxy10[0]<x+w) and (y<lxy10[1]<y+h) ):
        box2=[lxy10[0],lxy10[1],lxy10[0]+20,lxy10[1]+20]
        iou_s = Iox(box1,box2)
        if iou_s >0.5:
            print('第 10 格 偵測到 ')
    ...
    if ((x<lxy11[0]<x+w) and (y<lxy11[1]<y+h) ):
        box2=[lxy11[0],lxy11[1],lxy11[0]+20,lxy11[1]+20]
        iou_s = Iox(box1,box2)
        if iou_s >0.5:
            print('第 11 格 偵測到 ')
    ...
    if ((x<lxy12[0]<x+w) and (y<lxy12[1]<y+h) ):
        box2=[lxy12[0],lxy12[1],lxy12[0]+20,lxy12[1]+20]
        iou_s = Iox(box1,box2)
        if iou_s >0.5:
            print('第 12 格 偵測到 ')
    ...
    if ((x<lxy13[0]<x+w) and (y<lxy13[1]<y+h) ):
        box2=[lxy13[0],lxy13[1],lxy13[0]+20,lxy13[1]+20]
        iou_s = Iox(box1,box2)
        if iou_s >0.5:
            print('第 13 格 偵測到 ')
    ...
    if ((x<lxy14[0]<x+w) and (y<lxy14[1]<y+h) ):
        box2=[lxy14[0],lxy14[1],lxy14[0]+20,lxy14[1]+20]
        iou_s = Iox(box1,box2)
        if iou_s >0.5:
            print('第 14 格 偵測到 ')
    ...
    if ((x<lxy15[0]<x+w) and (y<lxy15[1]<y+h) ):
        box2=[lxy15[0],lxy15[1],lxy15[0]+20,lxy15[1]+20]
        iou_s = Iox(box1,box2)
        if iou_s >0.5:
            print('第 15 格 偵測到 ')
    ...
    if ((x<lxy16[0]<x+w) and (y<lxy16[1]<y+h) ):
        box2=[lxy16[0],lxy16[1],lxy16[0]+20,lxy16[1]+20]
        iou_s = Iox(box1,box2)
        if iou_s >0.5:
            print('第 16 格 偵測到 ')
...
def yolo_dec2(frame,classes, scores, boxes,box_count,nonum ,mode_1_1,flag,frame12):
    flag = False
    score2=0
    posi = []
    for (classid, score, box) in zip(classes, scores, boxes):
...
def sle (cnum,nonum,mode,mode_1_1,f):
    # print('\n 請選擇積木放置位置 : ')
    # print('\n 右邊棋盤選擇 : 1 ')
    # print('\n 左邊棋盤選擇 : 2 ')
    # text = '請選擇積木放置位置'
...
def main_strat(score2,cnum,nonum,f,mode,mode_1_1,time_flag,flag,vc,timer1,start_time): 
    cnum = int(cnum)
    #while time_flag==True :
    if time_flag==True :
        ...
        if mode =='1':
            ...
            flag2,frame,posi2=yolo_dec2(frame,classes, scores, boxes,box_count,nonum=nonum,mode_1_1=mode_1_1,flag=flag,frame12=frame12) 
            #posi =posi[0]
            posi3 = 0
            if len(posi2)>=1:
                posi3=posi2[-1]
            ...
            if flag2 == True:
                ...
                SS="INSERT INTO `blockbd`(`Score`,`Time`,`Mode`,`rl`,`nonum`,`cnum`,`boxes`,`time_`,`posi`) VALUES(%d,%d,%s,%s,%s,%s,%s,%s,%s)" %(score2,alltime,mode,mode_1_1,str(nonum),str(cnum),str(box_count),t,str(posi3))
                cursor.execute(SS)
                connection.commit()
        ...
        return frame,frame12,time_flag,int(alltime),score2
